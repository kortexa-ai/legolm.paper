...
\begin{{itemize}}[leftmargin=*]
  ...
  \item A characterization of hypernetwork weight collapse: diversity regularization makes generated weights input-dependent, and prolonged unregularized training can erase task-level conditioning, since the collapse threshold moves with the training budget{scaling_ref}.
\end{{itemize}}
...
\section{{Limitations}}
The bridge is large relative to the base model ({st['bridge_params'] / 1e6:.1f}M trainable parameters generating a {st['lora_dim']:,}-dimensional LoRA vector for a {facts['params_m']:.1f}M-parameter LM), so bridge compression is a central requirement for scaling this interface, not a footnote. All results are on one small base model trained on one corpus; the mechanism findings (capacity/conditioning dissociation, collapse) are the claims we expect to transfer, not the specific deltas. Scale-dependent results such as the prefix degradation should be re-established per model. The task probes use a single prompt template per modality and modest eval subsets. The budget-scaling run shows that probe outcomes can depend strongly on training length, so conclusions drawn at any single budget, including ours, should be read against that section. Composition of independently trained bridges remains sub-additive, and we do not yet offer a fix.
