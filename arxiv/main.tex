...
\begin{itemize}[leftmargin=*]
  ...
  \item A characterization of hypernetwork weight collapse: diversity regularization makes generated weights input-dependent, and prolonged unregularized training can erase task-level conditioning, since the collapse threshold moves with the training budget (Section~\ref{sec:scaling}).
\end{itemize}
...
The full run used rank 4, target set all, bridge learning rate 0.001, 300 benchmark training steps, 150 composition steps per brick, 600 task-probe training steps, and seeds 42, 1042, 2042. Benchmark, control, and task bridges train without a diversity penalty ($\lambda = 0$); Section~\ref{sec:diversity} sweeps $\lambda$. The result artifact is the summary JSON consumed by the figure and manuscript renderers.
...
\label{sec:diversity}
Table~\ref{tab:diversity} and Figure~\ref{fig:diversity} measure the generated-weight collapse directly. With no regularization, cross-input cosine similarity is close to one. Moderate diversity penalties reduce this collapse and make generated LoRA weights more input-dependent, with the lowest cross-input cosine at $\lambda=0.10$ (0.4421); the effect is not monotonic at the largest tested weight ($\lambda=0.20$ reverses to 0.5493 while remaining far below the unregularized value). The largest tested penalty eliminates the BPB gain (+0.0000 at $\lambda=0.20$, down from +0.0026 at $\lambda=0$): forcing input-dependent weight generation removes the text-loss gain, reinforcing the capacity/conditioning decomposition: the BPB improvement and input-dependent sensor use are separable objectives.
...
\section{Limitations}
The bridge is large relative to the base model (84.3M trainable parameters generating a 327,680-dimensional LoRA vector for a 33.6M-parameter LM), so bridge compression is a central requirement for scaling this interface, not a footnote. All results are on one small base model trained on one corpus; the mechanism findings (capacity/conditioning dissociation, collapse) are the claims we expect to transfer, not the specific deltas. Scale-dependent results such as the prefix degradation should be re-established per model. The task probes use a single prompt template per modality and modest eval subsets. The budget-scaling run shows that probe outcomes can depend strongly on training length, so conclusions drawn at any single budget, including ours, should be read against that section. Composition of independently trained bridges remains sub-additive, and we do not yet offer a fix.
...
\begin{verbatim}
paper-reproduce --suite all --include-audio-task \
  --benchmark-steps 300 --composition-steps-per-brick 150 \
  --task-steps 600 --eval-tokens 32768 --sensor-limit 64 \
  --max-eval-items 64 --seed 42 --output-dir results/<run>
\end{verbatim}
